...
const pricingPlans: PricingPlan[] = [
  ...
  },
  {
    ...
    features: [
      ...
      { name: "Домашние задания и проверка", included: true },
      { name: "Регулярные отчеты о прогрессе", included: true },
            ...
            >
              {plan.popular && (
                <div className="absolute top-0 right-0">
                  <div className="bg-gradient-to-r from-primary to-secondary text-white text-xs font-bold py-1 px-3 rounded-bl-lg">
                    Популярный выбор
                  </div>
                </div>
              )}
              ...
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-xl md:text-2xl font-montserrat">
                  ...
                  {plan.title === "Индивидуальные онлайн занятия" && <Video className="w-5 h-5 text-primary" />}
                  {plan.title === "Индивидуальные офлайн занятия" && <Users className="w-5 h-5 text-primary" />}
                  {plan.title}
                </CardTitle>
                <div className="mt-2">
